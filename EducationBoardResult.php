 ...
 * Handles official session, cookies and live scraping from eboardresults.com
 */
...
class EducationBoardResult {
    ...
    public function getCaptcha() {
        ...
        $ch = curl_init("{$this->baseUrl}/v2/home");
        ...
        // 2. Fetch the actual official binary captcha image from eboardresults.com
        $random = microtime(true) * 1000;
        $captchaUrl = "{$this->baseUrl}/v2/captcha?r={$random}";
        ...
        curl_setopt_array($ch2, [
            ...
            CURLOPT_SSL_VERIFYHOST => false,
            CURLOPT_COOKIEFILE => $this->cookieFile,
            CURLOPT_REFERER => "{$this->baseUrl}/v2/home",
            CURLOPT_TIMEOUT => 15,
            CURLOPT_CONNECTTIMEOUT => 10,
            CURLOPT_USERAGENT => 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/122.0.0.0 Safari/537.36',
            CURLOPT_HTTPHEADER => [
        ...
        return [
            'status' => 'error',
            'message' => 'শিক্ষা বোর্ডের কেন্দ্রীয় সার্ভার থেকে ক্যাপচা আনা যায়নি (' . ($curlError ?: "HTTP {$httpCode2}") . ')।'
        ];
    }
    ...
    public function fetchResult($board, $year, $roll, $reg, $captcha, $exam = 'ssc') {
        ...
        $ch = curl_init("{$this->baseUrl}/v2/getres");
        curl_setopt_array($ch, [
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_POST => true,
        ...
        return [
            'status' => 1,
            'msg' => 'শিক্ষা বোর্ডের কেন্দ্রীয় সার্ভার থেকে উত্তর পাওয়া যায়নি (' . ($curlError ?: "HTTP {$httpCode}") . ')।'
        ];
    }
}
